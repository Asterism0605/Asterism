...
describe('taxonomy', () => {
  ...
  it('SUBMEDIUM_BY_MEDIUM 涵蓋全部 4 個 medium，子類對齊 style-data.json', () => {
    expect(new Set(Object.keys(SUBMEDIUM_BY_MEDIUM))).toEqual(new Set(MEDIUM_LABELS));
    expect(new Set(SUBMEDIUM_BY_MEDIUM['Graphic Design'])).toEqual(
      new Set(['Brand Identity', 'Poster Design', 'Editorial Design', 'Packaging Design'])
    );
    expect(new Set(SUBMEDIUM_BY_MEDIUM['Outfit'])).toEqual(
      new Set(['Top', 'Bottom', 'Dress', 'Accessory'])
    );
    expect(new Set(SUBMEDIUM_BY_MEDIUM['Interior Design'])).toEqual(
      new Set(['Lighting', 'Table', 'Wall Paint', 'Chair'])
    );
    expect(new Set(SUBMEDIUM_BY_MEDIUM['Architecture'])).toEqual(
      new Set(['Window', 'Staircase', 'Facade', 'Entrance'])
    );
  });
