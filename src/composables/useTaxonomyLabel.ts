...
import { MEDIUM_ZH, STYLE_GROUP_ZH, STYLE_TAG_ZH, SUB_MEDIUM_EN, SUB_MEDIUM_ZH } from '@/data/styleLabels';

// medium / sub-medium / styleGroup 依語言顯示（zh 查對照表、en 回原文）。
...
export function useTaxonomyLabel() {
  ...
  const localizeTaxon = (value?: string | null): string => {
    ...
    return MEDIUM_ZH[value] ?? SUB_MEDIUM_ZH[value] ?? STYLE_GROUP_ZH[value] ?? STYLE_TAG_ZH[value] ?? value;
  };
